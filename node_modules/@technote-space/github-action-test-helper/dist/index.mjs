...
import { EOL } from 'os';
import { getOctokit as getOctokit$1 } from '@actions/github';
...
const testFs = (defaultExists = false) => {
    ...
    const setupMock = () => {
        ...
        spy.push(vi.spyOn(fs, 'existsSync').mockImplementation((path) => {
...
const getOctokit = (token) => getOctokit$1(token ?? 'test-token');

vi.mock('@actions/core');
const spyOnExportVariable = () => vi.spyOn(core, 'exportVariable').mockReturnValue();
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
const spyOnAddPath = () => vi.spyOn(core, 'addPath').mockReturnValue();
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
const spyOnSetOutput = () => vi.spyOn(core, 'setOutput').mockReturnValue();
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
const setupGlobal = () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const converter = (prefix = '') => (value) => process.stdout.write(prefix + JSON.stringify(value, null, '\t') + EOL);
...
export { addPathCalledWith, createResponse, disableNetConnect, encodeContent, execCalledWith, execContains, execNotContains, exportVariableCalledWith, generateContext, getApiFixture, getConfigFixture, getContext, getLogStdout, getOctokit, setActionEnv, setChildProcessParams, setOutputCalledWith, setupGlobal, spyOnAddPath, spyOnExec, spyOnExportVariable, spyOnSetOutput, spyOnSpawn, spyOnStdout, stdoutCalledWith, stdoutContains, stdoutNotContains, testChildProcess, testEnv, testFs, testProperties };
